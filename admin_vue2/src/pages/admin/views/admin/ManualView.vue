            ...
            <b-card class="mt-2">
                <div class="overflow-auto">
                    ...
                    <template v-slot:cell(1)='data'>
                        <a target="_blank" :href="`http://localhost:5005/download?filePath=${data.item[1]}`" v-if="!flag" > {{data.item[3]}} </a>
...
export default {
    ...
    created() {
        this.manualData = this.$route.params;
        this.manualSeq = this.$route.params.SEQ;
        this.ManualListDetail();
    },
    methods: {
        // 글 삭제
        delPosts(event) {
            ...
            if(question == true) {
                manualApi.deleteManualForm(this.manualSeq).then((res) => {
                    alert('삭제 되었습니다.')
        ...
        // 글 수정화면 이동
        setMode() {
            router.push(`/admin/manualform/${this.manualSeq}`)
